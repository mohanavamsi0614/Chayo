...
function Username() {
  ...
  function login() {
    if (value && user) {
      setload(true);
      axios
        .post("https://s60-mohanavamsi-chayo-2ovy.onrender.com/firebase", {
          ...data,
          username: user,
          photo: value,
        })
    ...
    reader.onload = async function (e) {
      try {
        const response = await axios.post(
          ...
          { file: e.target.result, upload_preset: "vh0llv8b" }
        );
  ...
  return (
    <div className=" h-screen bg-gray-950 flex flex-col items-center justify-center">
      <input
        className="bg-slate-900 text-white w-56
      ...
      />
      <button
        className=" w-48 mt-3 h-12 border hover:bg-white hover:text-black border-white text-white"
        onClick={login}
      >
        Submit
      </button>
      {load && (
        <div className="w-full gap-x-2 mt-4 flex justify-center items-center">
